...
if [[ "${CHANGELOG}" ]]
then
    appstreamcli news-to-metainfo ./NEWS.yaml ./org.liri.liri.appdata.xml
    python tools/fdroid_create_fastlane_changelogs.py

    git add fastlane/metadata/android/en-US/changelogs/ org.liri.liri.appdata.xml && git commit -m "update changelog for ${major}.${minor}.${patch}"
fi
